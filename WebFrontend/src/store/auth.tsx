...
function decodeJwtPayload(token: string): any | null {
  try {
    ...
    // atob may throw for non-padded base64
...
export function AuthProvider({ children }: { children: React.ReactNode }): JSX.Element {
  ...
  const [accessToken, setAccessToken] = useState<string | null>(() => (initialRemember ? localStorage.getItem(AT_KEY) : null));
  const [refreshToken, setRefreshToken] = useState<string | null>(() => (initialRemember ? localStorage.getItem(RT_KEY) : null));
  ...
  const persistOrClear = (at: string | null, rt: string | null, persist: boolean) => {
    try {
      if (persist) {
        if (at) localStorage.setItem(AT_KEY, at); else localStorage.removeItem(AT_KEY);
  ...
  const value = useMemo<AuthContextType>(() => ({
    ...
    login,
    logout
  }), [accessToken, refreshToken, roles, remember]);
